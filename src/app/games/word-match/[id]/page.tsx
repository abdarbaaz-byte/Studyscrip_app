...
export default function WordMatchTopicPage() {
  ...
  useEffect(() => {
    async function loadGameData() {
        ...
        const foundGame = allGames.find(g => g.id === gameId && g.type === 'WordMatch');
        if (foundGame && foundGame.pairs) {
            setGame(foundGame);
            setInitialWordPairs(foundGame.pairs);
        } else {
            notFound();
        }
        setLoading(false);
    ...
    <div className="container mx-auto px-4 py-8">
      <Card className="max-w-4xl mx-auto shadow-lg overflow-hidden bg-gradient-to-br from-secondary to-background">
        <CardHeader className="text-center bg-background/50 p-6">
          ...
          <CardTitle className="font-headline text-3xl md:text-4xl">{game.title}</CardTitle>
